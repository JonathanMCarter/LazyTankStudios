...
if (myClips[i] != null)
if (myClips[i].name == audioName) Playing(i, false);}}
public void Play(string audioName, bool Loop = false) {
for (int i = 0; i < myClips.Count; i++) {
if (myClips[i] != null)
if (myClips[i].name == audioName) Playing(i, Loop);}}
void Playing(int number, bool Loop) {
...
for (int i = 0; i < myClips.Count; i++) {
if (myClips[i] != null)
if (myClips[i].name == audioName) {
aud[0].clip = myClips[i];
aud[0].Play();}}}}