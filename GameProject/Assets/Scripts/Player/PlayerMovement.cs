...
public class PlayerMovement : A
{
  ...
  //  public Inventory i; //please dont name variables as one letter. we need to be able to clearly know what something is atm. Comment added by LC
    ...
    void FixedUpdate()
    {
        ...
        {
            myRigid.velocity = (new Vector2(IM.X_Axis(), IM.Y_Axis())).normalized * speed; //changed to fixedupdate as better for physics. Was also causing speed to fluxuate with framerate due to time.deltatime used incorrectly.
        }
        else
        {
